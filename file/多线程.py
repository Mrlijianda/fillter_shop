...
class Worker(QThread, QWidget):
    ...
    def start_filter(self):
        try:
            ...
            for num, url in enumerate(url_list):
                try:
                    ...
                    if spmc:
                        ...
                        self.log_textBrowser.append('当前商品------>', str(spmc).replace("['", '').replace("']", ''))
                        ...
                        if item11:
                            ...
                            self.log_textBrowser.append('当前未知产品数据----->', response.text)
                    ...
                    # 商品信息获取失败 更换代理IP继续后面采集(当前失败商品无法重新采集，已做保存处理)
                        ...
                        self.log_textBrowser.append('商品信息获取失败!更换代理IP继续后面采集(当前失败商品无法重新采集，已做保存处理)', url)
                    ...
                    self.log_textBrowser.append(e)
                    print(e)
                    self.log_textBrowser.append('IP过期,更换代理IP继续后面采集(当前失败商品无法重新采集，已做保存处理)')
            ...
            self.log_textBrowser.append('start_filter内出问题，请检查！')
    ...
    def taskstart(self):
        try:
            ...
            self.worker = Worker(self.key_1_edi, self.key_2_edi, self.key_3_edi, self.key_4_edi, self.key_5_edi,
                                 self.key_6_edi, self.key_7_edi, self.key_8_edi, self.tgid_edi, self.tgsqm_edi, self.log_textBrowser)
